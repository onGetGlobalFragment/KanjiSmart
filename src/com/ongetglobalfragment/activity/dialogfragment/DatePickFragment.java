...
public class DatePickFragment extends DialogFragment {
	...
	public DatePickFragment(String title,String ymd,String place,boolean isAdd) {
		this.title=title;
		this.ymd = ymd;
		this.place = place;
		this.ut = new Utils();
...
//		.setTitle(title)
	...
	private void initDatePicker(String ymd){
		
		if(ymd.isEmpty())ymd=ut.getTodsaySlashYmd();
		if(ymd.length()>8)ymd=ut.getPlaneYmdFromSlashYmd(ymd);
		
		CharSequence cs =null;
		cs = ymd.subSequence(0, ymd.length());
		...
		sb.append(cs.charAt(4)).append(cs.charAt(5));
		month = Integer.parseInt(sb.toString());
		sb = new StringBuilder();
		sb.append(cs.charAt(6)).append(cs.charAt(7));
		day = Integer.parseInt(sb.toString());
		dtp.updateDate(year, month-1, day);
